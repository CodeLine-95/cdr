...
App({
  ...
  _get(url, data, success, fail, complete, check_login) {
    ...
    let openId = wx.getStorageSync(uid).openId;
    let time = util.timeTamp(new Date());
    let sign = md5.hex_md5('openId='+openId+'&time='+time+'&uid='+uid+'&'+'zD7fN2mL4uP3lM5yQ7hI5wF');
  ...
  getPhoneNumber(e, callback) {
    ...
    wx.login({
      success(res) {
        // 发送用户信息
        App._post_form('/app/account/verification', {
          code: res.code,
          encryptedData: e.detail.encryptedData,
          iv: e.detail.iv,
        }, result => {
          // 记录token user_id openId
          wx.setStorageSync('token', result.data.token);
